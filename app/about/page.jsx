  ...
  const experienceData = [
    {
      ...
      logo: "/gemini_logo.png", // Update with actual logo path
      positions: [
        {
            ...
            <div className="border-l-2 border-yellow-400 relative pl-8">
              {experienceData[0].positions.map((position, index) => (
                <div key={index} className="relative mb-12">
                  ...
                  <h3 className="text-2xl font-semibold text-yellow-400">{position.role}</h3>
                  <p className="text-gray-300">{position.duration}</p>
                  <p className="text-sm text-gray-400 mb-2">{position.location}</p>
                  <p className="text-sm text-gray-100"><b>Skills:</b> {position.skills.join(" · ")}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="md:w-1/2">
          ...
          <div className="grid grid-cols-3 md:grid-cols-3 gap-6 text-center">
            {[{
              ...
              <div key={i} className="flex flex-col items-center transform transition-all hover:scale-105 hover:rotate-3 duration-150 ease-in-out">
